...
export function ConfirmDelete({ message, onConfirm, onCancel }) {
  return (
    <div className="overlay">
      <div style={{ background:'var(--bg-secondary)', borderRadius:16, padding:28, maxWidth:360, width:'92%', textAlign:'center', boxShadow:'0 8px 32px rgba(0,0,0,.14)', border:'1px solid var(--border-color)' }}>
        <div style={{ width:48, height:48, background:'#fee2e2', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px', color:'#dc2626' }}><Trash2 size={20}/></div>
        <p style={{ fontWeight:700, color:'var(--text-primary)', fontSize:15, marginBottom:8 }}>Confirmar operação</p>
        <p style={{ fontSize:13, color:'var(--text-secondary)', marginBottom:22 }}>{message}</p>
        <div style={{ display:'flex', gap:10, justifyContent:'center' }}>
          <button onClick={onCancel} style={{ padding:'9px 20px', border:'1px solid var(--border-color)', borderRadius:8, background:'var(--bg-secondary)', fontSize:13, fontWeight:500, cursor:'pointer', color:'var(--text-primary)' }}>Cancelar</button>
          <button onClick={onConfirm} style={{ padding:'9px 20px', border:'none', borderRadius:8, background:'#dc2626', color:'#fff', fontSize:13, fontWeight:600, cursor:'pointer' }}>Confirmar</button>
        </div>
      </div>
    </div>
  )
...
export function Toast({ message, type }) {
  return <div className="toast" style={{ background:type === 'ok' ? '#16a34a' : '#dc2626' }}>{message}</div>
}
...
export const BtnSecondary = ({ onClick, children, style={} }) => (
  <button onClick={onClick} style={{ background:'var(--bg-secondary)', color:'var(--text-primary)', border:'1px solid var(--border-color)', borderRadius:8, padding:'10px 16px', fontSize:14, fontWeight:500, cursor:'pointer', ...style }}>{children}</button>
)
...
export function Avatar({ name, size=28 }) {
  ...
  return <div style={{ width:size, height:size, borderRadius:'50%', background:'#dbeafe', color:'#1d4ed8', display:'flex', alignItems:'center', justifyContent:'center', fontSize:size*.35, fontWeight:700, flexShrink:0 }}>{initials}</div>
}

export function SectionHeader({ title, subtitle, action }) {
  return (
    <div className="section-header">
      <div><h2 className="section-title">{title}</h2>{subtitle && <p className="section-subtitle">{subtitle}</p>}</div>
      {action}
    </div>
  )
}
...
export function SearchInput({ value, onChange, placeholder }) {
  return (
    <div style={{ position:'relative', marginBottom:14 }}>
      <svg viewBox="0 0 24 24" width={14} height={14} fill="none" stroke="var(--text-secondary)" strokeWidth={2} style={{ position:'absolute', left:11, top:'50%', transform:'translateY(-50%)' }}>
        <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
      </svg>
      <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder || 'Buscar...'} style={{ paddingLeft:32, fontSize:13 }}/>
    </div>
  )
}
