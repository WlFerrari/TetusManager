...
class UsuariosController {
  ...
  async create(req, res, next) {
    try {
      ...
      if (req.body.senha && req.body.senha.length < 6) {
        return res.status(400).json({ ok: false, msg: 'Senha inicial: mínimo 6 caracteres.' })
      }
  ...
  async update(req, res, next) {
    try {
      ...
      const deixaDeSerAdminAtivo = atual.perfil === 'Administrador' && atual.status === 'Ativo'
        && (req.body.perfil !== 'Administrador' || req.body.status !== 'Ativo')
      ...
      const emailDuplicado = await UserRepo.findByEmail(req.body.email)
  ...
  async changePassword(req, res, next) {
    try {
      ...
      if (!senhaAtual || !novaSenha) return res.status(400).json({ ok: false, msg: 'Senhas são obrigatórias.' })
      if (novaSenha.length < 6) return res.status(400).json({ ok: false, msg: 'Nova senha: mínimo 6 caracteres.' })
